    ...
    try {
        ...
        if (!money || !(money >= minimumMoneyAllowed)) {
            return res.status(400).json({
                message: `Money is Required and it should be RM${minimumMoneyAllowed} or above!`,
                status: false,
                timeStamp: timeNow,
            })
        }
...
const addManualUSDTPaymentRequest = async (req, res) => {
    try {
        ...
        if (!money || !(money >= minimumMoneyAllowed)) {
            return res.status(400).json({
                message: `Money is Required and it should be RM${minimumMoneyAllowed} or ${(minimumMoneyAllowed / 92).toFixed(2)} or above!`,
                status: false,
                timeStamp: timeNow,
            })
        }
